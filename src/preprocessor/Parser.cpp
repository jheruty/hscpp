        ...
        while (!IsAtEnd())
        {
            if (pRequire->token.type == Token::Type::HscppRequirePreprocessorDef)
            {
                ...
                if (Peek().type != Token::Type::String && Peek().type != Token::Type::Identifier)
                {
                    ThrowError(LangError(
                            LangError::Code::Parser_HscppRequireExpectedStringLiteralOrIdentifierInArgumentList,
                            Peek().line, { pRequire->token.value }));
                }
            }
            else
            {
                Expect(Token::Type::String,
                        LangError(LangError::Code::Parser_HscppRequireExpectedStringLiteralInArgumentList,
                        Peek().line, { pRequire->token.value }));
            }
            ...
            {
                Expect(Token::Type::Comma, LangError(LangError::Code::Parser_HscppRequireMissingCommaInArgumentList,
